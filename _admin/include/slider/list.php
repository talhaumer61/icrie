<?php
   ...
   if(!LMS_VIEW && !isset($_GET['id'])) {  
      ...
      $sqllms  = $dblms->querylms("SELECT slider_id, slider_status, slider_img, slider_title, slider_btn_href, slider_btn_text
                                    FROM ".SLIDER." 
                                    WHERE slider_id != '' AND is_deleted = '0' 
                                    $sql2
      ...
      $sqllms  = $dblms->querylms("SELECT slider_id, slider_status, slider_img, slider_title, slider_btn_href, slider_btn_text
                                    ...
                                    WHERE slider_id != '' AND is_deleted = '0'
                                    $sql2
                                    ORDER BY slider_id DESC LIMIT ".((int)$page-1)*$Limit .",$Limit
                                 ");
      echo'
      <div class="row">
         <div class="col-lg-12">
            <div class="card">
               <div class="card-body">
                  <h4 class="card-title mb-4 float-left"> <i class="fa fa-file-image"></i> Sliders List</h4>
                  <div class="card-title mb-4 float-right"> 
                     <a href="slider.php" class="btn btn-primary"><i class="fa fa-list"></i> All </a>
                           ...
                           while($value_slider = mysqli_fetch_array($sqllms)) {
                              ...
                              <tr>
                                 ...
                                 <td class="text-center">
                                    <img src="../images/slider/'.$value_slider['slider_img'].'" alt="'.$value_slider['slider_title'].'" class="rounded-circle avatar-sm">
                                 </td>
                                 <td>'.$value_slider['slider_title'].'</td>
                                 <td>'.$value_slider['slider_btn_text'].'</td>
                                 <td>'.$value_slider['slider_btn_href'].'</td>
                                 <td class="text-center">'.get_admstatus($value_slider['slider_status']).'</td>
                                 <td class="text-center">
                                    <a href="?id='.$value_slider['slider_id'].'" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i></a> 
                                    ...
                                    <div class="modal-content">
                                       <form method="POST" action="slider.php">
                                          ...
                                          <div class="modal-body">
                                             <input type="hidden" name="slider_id" value="'.$value_slider['slider_id'].'">
                                             <p>Are you sure you want to delete slider:</p>
                                             <h5>'.$value_slider['slider_title'].'</h5>
                                          </div>
                                          <div class="modal-footer">
                                             <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                             <button type="submit" name="Slider_delete" class="btn btn-danger">Delete</button>
                                          </div>
                                       </form>
                                    </div>
                                 </div>

<?php
$condition = array(
    'select'       =>  'functions_id, functions_status, functions_title, id_type, id_sub_type, id_team, functions_desc'
   ,'where'        =>  array(
                                'is_deleted'   => 0
                               ,'functions_status'   => 1
                               ,'functions_id'       => cleanvars(LMS_EDIT_ID)
                           )
   ,'return_type'  =>  'single'
);
...
<div class="card">
    <form autocomplete="off" class="form-validate" enctype="multipart/form-data" method="post" accept-charset="utf-8">
        <div class="card-body">
            <div class="row">
                <input type="hidden" name="functions_id" value="'.$row['functions_id'].'" />
                <div class="col mb-2">
                    <label class="form-label">Title <span class="text-danger">*</span></label>
                    <input type="text" class="form-control" name="functions_title" value="'.$row['functions_title'].'" required />
                </div>
                <div class="col mb-2">
            ...
            <div class="row">
                <div class="col mb-2">
                    ...
                    <select class="form-control js-example-basic-single" name="functions_status" id="news_status" required>
                        <option value=""> Choose one</option>';
                        foreach(get_status() as $key => $status):
                            echo'<option value="'.$key.'" '.($key == $row['functions_status'] ? 'selected' : '').'>'.$status.'</option>';
                        endforeach;
                        echo'
                    </select>
                </div>
            ...
            <div class="row">
                <div class="col mb-2">
                    ...
                    <textarea class="form-control" id="ckeditor1" name="functions_desc">'.$row['functions_desc'].'</textarea>
                </div>
            </div>
            <div class="row">
                <div class="col mb-2">
                ...
                <div class="col mb-2">
                    ...
                    <div class="dropzone" id="singleFileUpload">
                        <div class="dz-message needsclick">
                            <i class="fa fa-cloud-upload"></i>
                            <h6>Drop files here or click to upload.</h6>
                            <input type="file" class="form-control" name="functions_file" id="news_photo" accept=".docx, .pdf, .pptx, .doc" required>
                        </div>
                    </div>
                </div>
            </div>

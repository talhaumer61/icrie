<?php
$condition = array(
    'select'       =>  'journal_id, journal_status, journal_title id_team, journal_desc'
   ,'where'        =>  array(
                                'is_deleted'   => 0
                               ,'journal_status'   => 1
                               ,'journal_id'       => cleanvars(LMS_EDIT_ID)
                           )
   ,'return_type'  =>  'single'
);
$row = $dblms->getRows(JOURNALS , $condition);
echo'
<div class="row">
    <div class="col-sm-6">
...
<div class="card">
    <form autocomplete="off" class="form-validate" enctype="multipart/form-data" method="post" accept-charset="utf-8">
        <div class="card-body">
            <div class="row">
                <input type="hidden" name="journal_id" value="'.$row['journal_id'].'" />
                <div class="col mb-2">
                    <label class="form-label">Title <span class="text-danger">*</span></label>
                    <input type="text" class="form-control" name="journal_title" value="'.$row['journal_title'].'" required />
                </div>
            </div>
            <div class="row">
                <div class="col mb-2">
                    <label class="form-label">Status <span class="text-danger">*</span></label>
                    <select class="form-control js-example-basic-single" name="journal_status" id="news_status" required>
                        <option value=""> Choose one</option>';
                        foreach(get_status() as $key => $status):
                            echo'<option value="'.$key.'" '.($key == $row['journal_status'] ? 'selected' : '').'>'.$status.'</option>';
                        endforeach;
                        echo'
                    </select>
                </div>
            ...
            <div class="row">
                <div class="col mb-2">
                    ...
                    <textarea class="form-control" id="ckeditor1" name="journal_desc">'.$row['journal_desc'].'</textarea>
                </div>
            </div>
            <div class="row">
                <div class="col mb-2">
                    ...
                    <div class="dropzone" id="singleFileUpload">
                        <div class="dz-message needsclick">
                            ...
                            <input type="file" class="form-control" name="journal_photo" id="news_photo" accept=".jpg, .jpeg, .png" required>
                        </div>
                    </div>
                </div>
                <div class="col mb-2">
                    ...
                    <div class="dropzone" id="singleFileUpload">
                        <div class="dz-message needsclick">
                            <i class="fa fa-cloud-upload"></i>
                            <h6>Drop files here or click to upload.</h6>
                            <input type="file" class="form-control" name="journal_file" id="news_photo" accept=".docx, .pdf, .pptx, .doc" required>
                        </div>
                    </div>
                </div>
            </div>
